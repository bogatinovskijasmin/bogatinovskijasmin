...
9) Laplacian Eigenmaps [LE](https://www2.imm.dtu.dk/projects/manifold/Papers/Laplacian.pdf),

10) Maximal Variance Unfolding [(MVU)](https://repository.upenn.edu/cgi/viewcontent.cgi?article=1001&context=cis_papers),
...
1) Make a linear kernel on $U^TX$ which is $K=X^TUU^TX$

2) Make a kernel B over Y

3) Write HSIC(K, B) = $\frac{1}{(n-2)^2}Tr(X^TUU^TXHBH)$, just U is unknown and we maximize HSIC with regard to U.
...
The following image depicts representations obtained with LE. The image is taken from the original [paper](https://www2.imm.dtu.dk/projects/manifold/Papers/Laplacian.pdf) where the method is introduced.

![image](../assets/img/representation_learning/LE.png)

### Maximal Variance Unfloding
...
The following image depicts representations obtained with MVU. The image was taken from the original [paper](https://repository.upenn.edu/cgi/viewcontent.cgi?article=1001&context=cis_papers).
![image](../assets/img/representation_learning/MVU.png)


### Nystrom approximation
One issue with all of the discussed methods is that they do not scale well. Nystrom approximation provides efficient workaround to make the methods more scalable.
...
\begin{equation}
D_{KL} = D_{KL}[P_s(s), P_{s}^{\*}(s)] = \int  P_s(s)ln\frac{P_s(s)}{\prod_{i=1}^N P_{s_i}^{\*}(s_i)} ds (1)
\end{equation}
...
In this post we provide extensive overview of many dimensionality reduction techniques. We demonstrate how the majority of the methods can be framed under the paradigm of Kernel PCA. Furthermore we showcased how KPCA relates to methods that are aiming to learn different representation e.g. via learning kernels or by minimizing (regularized) objective function. Finally, we show how to decompose either latent components or independent signals given their mixture. We finished with a visualization technique that allows to visualize high-dimensional data.
