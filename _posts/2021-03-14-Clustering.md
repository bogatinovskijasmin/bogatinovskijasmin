...
two disjoint sets. We denote the cut as $cut(A, A^{i})=\sum_{i,j}w_{ij}$,
where A and $A^{i}$ are two disjoint sets of verticies (data points) $i$ is
a point from set A, and $j$ is a point from set $A^{i}$ and $w_{ij}$ is a
distance measure between the points.

Given the graph $G=(V, E)$ where $V=A$U$A^{i}$ and $A$ and $A^{i}$ are
disjoint sets of verticies, the goal is to find the optimal set of
verticies such that $argmin_{A, A^{i}}cut(A, A^{i})=\sum_{i,j}w_{ij}$ is
