...
Kernel PCA is a method for dimenisionality reduction. It addresses the limitation of the linear PCA method. PCA assumes that the data exist on a linear manifold (or a subset of the manifold). To do this it uses the "kernel trick", as described above.
...
\end{equation}
then the reconsturiction error is given as: $o(x)=||\sum_{i=h}^du_iu_i^T\phi(x)||^2$.
For high dimensions, we can only compute empirical principal components. An interpolated projection on the ith empirical principal component is given by:
\begin{equation}
proj_i(x)=k(x,X)U_{:,i}\lambda_i^{-0.5}
\end{equation}
then $o(x)=||\sum_{i=h}^du_i^Tproj(x)^2$.

### Supprot vector data description (SVDD method) for anomaly detection
SVDD is another method that is based on SVM paradigm and it is used for anomaly detection (unsupervised). It assumes that the data lie within a spehere with radius R and center c. The points inside the spehere are considered as inliers (normal) while the points outside of the sphere are considered as outliers. We can defined constrained quadratic programming problem to perform this optimization.
...
on the fraction of support vectors. Furthermore, we can write the dual optmization problem from here as:
